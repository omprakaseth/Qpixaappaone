...
import { LayoutDashboard, Users, FileText, Flag, Coins, ArrowLeft, Shield, BarChart3, Bell, Settings, Megaphone, Zap, Menu, X, ShieldCheck, ChevronLeft, ClipboardList } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const SidebarContent = () => (
    <>
      <div className="p-4 border-b border-border flex items-center justify-between">
        ...
        {activeTab !== 'dashboard' && (
          <button onClick={handleBack} className="p-1.5 rounded-lg hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors">
            <ChevronLeft size={18} />
          </button>
        )}
      </div>
      ...
      <div className="p-3 border-t border-border">
        <button onClick={() => router.push('/')} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors">
          <ArrowLeft size={17} />
          Back to App
        </button>
      </div>
  ...
  return (
    <div className="h-screen bg-background flex overflow-hidden">
      ...
      <div ref={contentRef} className="flex-1 overflow-y-auto p-4 md:p-6 md:pt-24 pt-20 relative animate-in fade-in duration-300">
        {renderContent()}
      </div>
            ...
            <div className="flex gap-3">
              ...
              <button 
                onClick={() => {
                  ...
                  if (pendingTab) window.location.hash = pendingTab;
                }}
