...
export default function CatchAllPage() {
  ...
  // Deriving ID from slug for deeper routes
  const id = useMemo(() => {
    if (!slug) return undefined;
    if (slug.length >= 2) {
      if (slug[0] === 'prompt' || slug[0] === 'creator') {
        return slug[1];
      }
    }
    return undefined;
  ...
  // Sync state with URL only if not already matched to prevent flicker
  useEffect(() => {
    const syncWithUrl = async () => {
      if (pathname && pathname.startsWith('/prompt/') && id) {
      ...
      if (pathname && pathname.startsWith('/creator/') && id) {
        if (viewingCreator !== id) setViewingCreator(id);
      } else {
        if (viewingCreator) setViewingCreator(null);
      }
    };
    syncWithUrl();
  }, [pathname, id, posts, fetchPostById, selectedPost, viewingCreator]);
  ...
  const openPost = useCallback((post: Post) => {
    if (selectedPost?.id === post.id || isNavigatingRef.current) return;
